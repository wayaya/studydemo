...
TINY.slideshow.prototype = {init:function(s, z, b, f, q, tp, tn) {
    ...
    for (i; i < this.l; i++) {
        ...
        if (this.thumbs) {
            ...
            temp.setAttribute("src",g.getAttribute("src"));
    ...
    if (tp && tn) {
        ...
        tp.onclick = new Function(this.n + '.mv(-1,1)');
        tn.onclick = new Function(this.n + '.mv(1,1)')
    }
